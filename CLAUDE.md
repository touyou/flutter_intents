...
- **WWDC26 richer parameter types (#53, partial — `Duration` + `PersonName`)**
  ...
  - Both ride the `rich-types` flag and the shared `nativeRichTypes` branch flag; the native types must never leak into the default/`#else` output (no stable `_IntentValue` conformance → would not compile). Same `<field>…` normalized-local pattern as IntentFile's `<field>FileInfo`.
  - Verified: dual-branch `swiftc -typecheck` green for FlutterBridge + URL `Duration`/`PersonName` intents (fallback @ iOS 17, native @ iOS 27).
  - **Deferred (need a handler-I/F design, not pure codegen)**: `@UnionValue` (model the Dart side as a `sealed class` ADT), `EntityCollection<T>` (handler receives identifiers).

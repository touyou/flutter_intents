...
- `@IntentSpec` gains `resultDialogSupportingTemplate` and `resultDialogSystemImageName`, for `IntentDialog(full:supporting:)` result dialogs (ADR 0006). Both are additive and optional; existing specs are unaffected.

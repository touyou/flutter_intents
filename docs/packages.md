...
### Annotations for newer OS features (no opt-in needed)

These fields are generated by default and guarded by plain `@available`, because
the symbols they emit exist in the released SDK:
...
6. **WWDC26 experimental generation (opt-in, default OFF)** ✅
   ...
   - App Schema (#49), execution control (#52), rich parameter types (#53),
     cross-app export (#54), `SyncableEntity` / `RelevantEntities` donation
     (#55). See `docs/usage.md` and `docs/adr/`.
   - `IntentValueQuery` (#51) **graduated** out of this opt-in: the symbol is in
     the released SDK at iOS 26.0, so it is generated by default under plain
     `@available`.
   - Verification: `scripts/verify_experimental_swift.sh` dual-branch
     `swiftc -typecheck`. On Xcode 27 it checks both branches; on a stable Xcode
     it checks the stable branch only (switch with `DEVELOPER_DIR`).

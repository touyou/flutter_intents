...
The sources ship **inside the published `app_intents` pub package**, at
`ios/AppIntentsBridge/`. Pick whichever of the three routes matches your project;
all three build the exact same files, and the first two are version-locked to
the `app_intents` version already in your `pubspec.yaml`.

**A. Local Swift package (recommended, if your project still has a Podfile)**

1. Run `flutter pub get`, then `pod install` once (or `flutter build ios`) so
   that `ios/.symlinks/plugins/` exists.
2. In Xcode: **File → Add Package Dependencies… → Add Local…**
3. Choose `ios/.symlinks/plugins/app_intents/ios/AppIntentsBridge`
4. Add the `AppIntentsBridge` library to the target that needs it — the
   `Runner` target, a Widget Extension target, or both.
...
> **No Podfile? Use route C.** `ios/.symlinks/` is created by
> `flutter_install_all_ios_pods` while CocoaPods evaluates your Podfile, and by
> nothing else. A project that has fully migrated to Swift Package Manager and
> run `pod deintegrate` — which the Flutter tool itself suggests once every
> plugin is a Swift Package — has no Podfile, so `flutter pub get` and
> `flutter build ios` never create that directory. Routes A and B both depend on
> it; route C is the supported option there.

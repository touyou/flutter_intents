...
その後、向こうで**宣言を外した TestFlight ビルドで App Intents が再び取り込まれる**ことが
観測され、原因（`c4f20ef` の宣言追加）と対処（宣言の削除）の両方が実証された。
なお `extract.packagedata` のマングル名解決が壊れるという**機序**は、証拠と整合する説明で
あって直接観測したものではない。実証されているのは「宣言あり → 壊れる / 宣言なし → 直る」である。
